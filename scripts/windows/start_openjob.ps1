...
if ($PythonPath) {
	...
	$RunnerPrefix = @("-m", "openjob.main")
} else {
	$OpenJobCmd = Get-Command "openjob" -ErrorAction SilentlyContinue
	if ($OpenJobCmd) {
		$Runner = $OpenJobCmd.Source
		$RunnerPrefix = @()
	} else {
	...
	if (-not $Python) {
		throw "Could not find OpenJob or Python. Install the project first with: pip install -e ."
	}
...
$ChromeProfile = Join-Path $env:LOCALAPPDATA "OpenJobChrome"
$ChromeArguments = @(
...
if (-not $SkipChrome) {
	Write-Host "Starting the OpenJob Chrome profile..."
...
Write-Host "OpenJob is ready. Log in manually in the dedicated Chrome window if needed."
