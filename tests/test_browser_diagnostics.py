...
class BrowserDiagnosticsTests(unittest.TestCase):
    ...
    @patch("openjob.browser.diagnostics.find_boss_tab")
    @patch("openjob.browser.diagnostics.runtime_targets")
    @patch("openjob.browser.diagnostics.ensure_runtime")
    @patch("openjob.browser.diagnostics.check_node_available")
    def test_run_browser_diagnostics_reports_ready_runtime(self, check_node, ensure_runtime, runtime_targets, find_boss_tab):
        from openjob.browser.diagnostics import run_browser_diagnostics
    ...
    @patch("openjob.browser.diagnostics.find_boss_tab")
    @patch("openjob.browser.diagnostics.runtime_targets")
    @patch("openjob.browser.diagnostics.runtime_health")
    @patch("openjob.browser.diagnostics.ensure_runtime")
    @patch("openjob.browser.diagnostics.check_node_available")
    def test_run_browser_diagnostics_reports_updated_url_after_runtime_port_fallback(
        ...
        runtime_health.return_value = {"status": "ok", "runtime": "openjob"}
        runtime_targets.return_value = [{"targetId": "1", "url": "https://www.zhipin.com"}]
        find_boss_tab.return_value = {"targetId": "1", "title": "BOSS直聘"}
    ...
    @patch("openjob.browser.diagnostics.ensure_runtime")
    @patch("openjob.browser.diagnostics.check_node_available")
    def test_run_browser_diagnostics_reports_non_openjob_service_on_runtime_port(self, check_node, ensure_runtime, runtime_health):
        from openjob.browser.diagnostics import run_browser_diagnostics
        ...
        self.assertFalse(result["runtime"])
        self.assertTrue(any("non-OpenJob" in error for error in result["errors"]))

    @patch("openjob.browser.diagnostics.run_browser_diagnostics")
    def test_print_browser_diagnostics_shows_non_openjob_service_message(self, run_browser_diagnostics):
        from openjob.browser.diagnostics import print_browser_diagnostics
        ...
        run_browser_diagnostics.return_value = {
            ...
            "targets": [],
            "boss_tab": None,
            "errors": ["Runtime port is occupied by a non-OpenJob service."],
        ...
        self.assertIn("端口已被非 OpenJob 服务占用", buffer.getvalue())
