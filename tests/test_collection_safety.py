...
from openjob.config import DEFAULTS, load_config
...
from openjob.web.server import _execute_collect, _wait_for_collection_delivery_cooldown
...
""",
    ...
    def test_collection_risk_stops_and_records_safe_reason(self):
        ...
        with patch("openjob.scraper.jobs.get_db", return_value=db), \
             ...
             patch("openjob.scraper.jobs.Progress", return_value=context), \
             patch("openjob.scraper.jobs.new_tab", return_value="worker"), \
             patch("openjob.scraper.jobs.wait_for_load"), \
             patch("openjob.scraper.jobs.evaluate", return_value=json.dumps({"risk": "captcha"})), \
    ...
    def test_consecutive_page_failures_end_collection_without_risk_lock(self):
        ...
        with patch("openjob.scraper.jobs.get_db", return_value=db), \
             patch("openjob.collection.platforms.boss.PlatformAccessGuard") as guard_cls, \
             patch("openjob.scraper.jobs.Progress", return_value=context), \
             patch("openjob.scraper.jobs.new_tab", return_value=None), \
             patch("openjob.scraper.jobs.close_tab"), \
             patch("openjob.scraper.jobs.time.sleep"):
            count = scrape_jobs(config, ["AI"])
    ...
    def test_frontend_task_log_explains_daily_limit(self):
        ...
        with patch("openjob.scraper.jobs.scrape_jobs", side_effect=fake_scrape), \
             patch("openjob.ai.scorer.score_jobs", return_value=(0, 0)):
    ...
    def test_collection_delivery_cooldown_selects_one_random_value_per_flow(self):
        ...
        with patch("openjob.web.server.random.uniform", return_value=11.25) as choose:
