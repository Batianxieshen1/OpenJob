...
from openjob.throttle import SendWindowChecker
from openjob.web import server
from threading import Event, Lock

from openjob.web.tasks import TaskAlreadyRunningError, WorkbenchTask, WorkbenchTaskRunner
...
class WebApiRouteTests(unittest.TestCase):
    ...
    @patch("openjob.web.server.make_server")
    def test_run_server_serves_gzipped_threaded_wsgi(self, make_server):
        # Act
        server.run_server(open_browser=False)

        # Assert：仍使用线程化 WSGI 服务器，且 app 被 gzip 中间件包裹
        wrapped = make_server.call_args.args[2]
        self.assertIsInstance(wrapped, server._GzipMiddleware)
        self.assertTrue(issubclass(make_server.call_args.kwargs["server_class"], ThreadingMixIn))
        self.assertTrue(issubclass(make_server.call_args.kwargs["server_class"], WSGIServer))
        self.assertTrue(make_server.call_args.kwargs["server_class"].daemon_threads)
    ...
    def test_web_api_workbench_preflight_full_requires_ai_key(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            browser_ready = {
                ...
                "health": {"runtime": "openjob"},
                "browser_product": "Chrome/138.0",
            }
            with (
                patch.dict("os.environ", {}, clear=True),
                patch("openjob.web.preflight.run_browser_diagnostics", return_value=browser_ready),
            ):
                status, headers, body = self._request("/api/workbench/preflight?mode=full")
    ...
    def test_task_runner_automatically_stops_at_send_window_deadline(self):
        ...
        # Act
        with patch("openjob.web.tasks._deadline_from_config", return_value=deadline):
    ...
    def test_web_api_manual_sent_records_external_send_without_using_boss_quota(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            verify_db = get_db(base_dir / "data" / "openjob.db")
            try:
    ...
    def test_monitor_loop_processes_queued_delivery_before_next_check(self):
        ...
        with patch.object(server, "_execute_deliver") as execute_deliver, \
             patch(
                 "openjob.executor.monitor.monitor_and_send_resumes",
    ...
    def test_deliver_keeps_partial_result_and_continues_after_single_failure(self):
        ...
        with patch("openjob.ai.greeter.generate_greetings", return_value=3), \
             patch("openjob.executor.sender.send_greetings", side_effect=fake_send):
            server._execute_deliver(task, config)
    ...
    def test_deliver_still_stops_on_account_risk_signal(self):
        ...
        # Act / Assert
        with patch("openjob.ai.greeter.generate_greetings", return_value=2), \
             patch("openjob.executor.sender.send_greetings", side_effect=fake_send), \
    ...
    def test_web_api_workbench_reject_marks_selected_ready_jobs_rejected(self):
        ...
        self.assertEqual(payload["blocked_companies"], [])  # 未请求拉黑时不改黑名单
