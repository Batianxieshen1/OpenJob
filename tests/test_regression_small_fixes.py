...
class VersionMetadataTests(unittest.TestCase):
    def test_release_version_is_consistent(self):
        ...
        import openjob
        from openjob.web.server import health
...
class AiPromptRegressionTests(unittest.TestCase):
    def test_scorer_prompt_uses_universal_weighted_evidence(self):
        from openjob.ai.scorer import SCORING_PROMPT
    ...
    def test_tailored_resume_prompt_preserves_platform_growth_cases(self):
        from openjob.ai.resume_engine.optimizer import REWRITE_SYSTEM

        self.assertIn("平台案例与量化结果", REWRITE_SYSTEM)
        self.assertIn("阅读/观看", REWRITE_SYSTEM)
        self.assertIn("粉丝增长", REWRITE_SYSTEM)
...
class ConfirmationUiTests(unittest.TestCase):
    @patch("openjob.ui.confirm.Prompt.ask")
    @patch("openjob.ui.confirm.get_jobs_pending_confirmation")
    @patch("openjob.ui.confirm.get_db")
...
class ConfigSchemaTests(unittest.TestCase):
    def setUp(self):
        ...
        self.schema_source = (
            ROOT / "src" / "openjob" / "web" / "config_schema.json"
        ).read_text(encoding="utf-8")
...
class ScorerPrefilterTests(unittest.TestCase):
    def setUp(self):
        self.source = (ROOT / "src" / "openjob" / "ai" / "scorer.py").read_text(encoding="utf-8")
