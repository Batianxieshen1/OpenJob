...
def test_scoring_never_selects_soft_deleted_pending_jobs(tmp_path):
    ...
    with patch("openjob.ai.scorer.get_db", side_effect=lambda: get_db(db_path)), \
         patch("openjob.ai.scorer._load_resume", return_value="resume"), \
         patch("openjob.ai.scorer._request_score") as request_score:
