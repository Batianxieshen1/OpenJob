...
class WebConfigApiTests(unittest.TestCase):
	...
	def test_sanitize_config_defaults_to_deepseek_service(self):
		# Arrange
		with tempfile.TemporaryDirectory() as tmp:
			config_path = Path(tmp) / "config.yaml"
			config_path.write_text(yaml.dump({"ai": {}}, sort_keys=False), encoding="utf-8")

			# Act
			with patch.object(server, "CONFIG_PATH", config_path):
				cleaned = server._sanitize_config_for_write({"ai": {"provider": "unknown", "model": "deepseek-chat"}})

		# Assert
		self.assertEqual(cleaned["ai"]["service"], "deepseek")
		self.assertEqual(cleaned["ai"]["provider"], "openai_compatible")
		self.assertEqual(cleaned["ai"]["model"], "deepseek-chat")
