...
from openjob.db import add_history, get_db, insert_job, update_job_status
from openjob.throttle import RequestThrottle
...
class MonitorThrottleTests(unittest.TestCase):
    ...
    def test_mark_makes_configured_request_interval_effective(self):
        ...
        with patch("openjob.throttle.time.time", return_value=100), \
             patch("openjob.throttle.random.gauss", return_value=60), \
             patch("openjob.throttle.random.random", return_value=1), \
             patch("openjob.throttle.time.sleep") as sleep:
            throttle.mark()
...
class FullFlowMonitorCooldownTests(unittest.TestCase):
    def test_full_flow_initial_cooldown_is_cancellable_before_first_scan(self):
        from openjob.web.tasks import WorkbenchTask, wait_for_initial_monitor_cooldown
