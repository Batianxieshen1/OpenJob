...
from openjob.executor.sender import CHAT_BUTTON_SCRIPT_FOR_TESTS
from openjob.executor.sender import _chat_target_matches_job
from openjob.executor.sender import _confirm_preset_greeting
from openjob.executor.sender import _handle_greet_popup
from openjob.executor.sender import _message_delivery_state
from openjob.executor.sender import _verify_greeting_in_chat_list
from openjob.executor.sender import send_greetings
from openjob.executor.sender import _send_greeting_once
from openjob.executor.sender import _submit_chat_message_background
from openjob.executor.sender import _submit_startchat_greeting
from openjob.executor.sender import _wait_for_chat_page
...
class JobSelectionTests(unittest.TestCase):
    ...
    def test_preset_confirmation_uses_background_dom_click(self):
        with patch(
            "openjob.executor.sender.evaluate",
            return_value='{"success": true, "action": "preset_confirmed"}',
        ) as evaluate_mock, patch("openjob.executor.sender.click_at") as click_at:
    ...
    def test_startchat_submission_uses_trusted_typing_and_real_send_message_click(self):
        ...
        with patch(
            ...
            "openjob.executor.sender.click_at",
            return_value=True,
        ) as click_at, patch(
            "openjob.executor.sender.press_key",
            return_value=True,
        ) as press_key, patch(
            "openjob.executor.sender.type_text",
            return_value=True,
        ) as type_text:
    ...
    def test_chat_list_verification_requires_company_and_complete_greeting(self):
        ...
        with patch("openjob.executor.sender.new_tab", return_value="chat-target"), \
             patch("openjob.executor.sender.wait_for_load"), \
             patch("openjob.executor.sender.evaluate", return_value=result) as evaluate, \
             patch("openjob.executor.sender.close_tab") as close_tab:
    ...
    def test_startchat_popup_reuses_chat_redirect_without_foreground_input(self):
        ...
        with patch(
            "openjob.executor.sender._detect_greet_popup",
            return_value={"success": True, "popup": True, "kind": "startchat_dialog"},
        ), patch(
            "openjob.executor.sender._navigate_to_chat_redirect",
            return_value=True,
        ) as redirect, patch(
            "openjob.executor.sender._submit_startchat_greeting",
    ...
    def test_send_greeting_reports_unavailable_job_page_before_clicking_chat(self):
        ...
        with patch("openjob.executor.sender.new_tab", return_value="target-1") as new_tab, \
             patch("openjob.executor.sender.evaluate", return_value='{"success": false, "error": "job_page_unavailable", "history_detail": "岗位页面不存在或已下架", "skip_backoff": true}'), \
             patch("openjob.executor.sender.close_tab") as close_tab, \
             patch("openjob.executor.sender.time.sleep"):
    ...
    def test_send_greeting_uses_real_click_fallback_when_chat_button_does_not_navigate(self):
        ...
        with patch("openjob.executor.sender.new_tab", return_value="target-1"), \
             patch("openjob.executor.sender.evaluate", return_value='{"success": true}'), \
             patch("openjob.executor.sender._click_chat_button", return_value={"success": True, "button_text": "继续沟通"}), \
             patch("openjob.executor.sender._detect_greet_popup", return_value={"success": True, "popup": False}), \
             patch("openjob.executor.sender._wait_for_chat_page", side_effect=[
                 {"success": False, "error": "chat_navigation_timeout"},
                 {"success": True, "target_id": "target-1"},
             ]), \
             patch("openjob.executor.sender._message_delivery_state", side_effect=["missing", "delivered", "delivered"]), \
             patch("openjob.executor.sender._submit_chat_message_background", return_value={"success": False}), \
             patch("openjob.executor.sender._fill_chat_input", return_value={"success": True, "disabled": False}), \
             patch("openjob.executor.sender.click_at", return_value=True) as click_at, \
             patch("openjob.executor.sender.close_tab") as close_tab, \
             patch("openjob.executor.sender.time.sleep"):
            result, target_id = _send_greeting_once(
                job,
    ...
    def test_send_greeting_waits_for_chat_button_before_failing(self):
        ...
        with patch("openjob.executor.sender.new_tab", return_value="target-1"), \
             patch("openjob.executor.sender.evaluate", side_effect=evaluate_results) as evaluate_mock, \
             patch("openjob.executor.sender._detect_greet_popup", return_value={"success": True, "popup": False}), \
             patch("openjob.executor.sender._wait_for_chat_page", return_value={"success": True, "target_id": "target-1"}), \
             patch("openjob.executor.sender._message_delivery_state", side_effect=["missing", "delivered", "delivered"]), \
             patch("openjob.executor.sender._has_outgoing_messages", return_value=False), \
             patch("openjob.executor.sender._submit_chat_message_background", return_value={"success": False}), \
             patch("openjob.executor.sender._fill_chat_input", return_value={"success": True, "disabled": False}), \
             patch("openjob.executor.sender.click_at", return_value=True), \
             patch("openjob.executor.sender.close_tab") as close_tab, \
             patch("openjob.executor.sender.time.sleep"):
            result, target_id = _send_greeting_once(
                job,
    ...
    def test_send_greeting_accepts_chat_list_receipt_when_echo_is_missing(self):
        ...
        with patch("openjob.executor.sender.new_tab", return_value="target-1"), \
             ...
             patch("openjob.executor.sender._detect_greet_popup", return_value={"success": True, "popup": False}), \
             patch("openjob.executor.sender._wait_for_chat_page", return_value={"success": True, "target_id": "target-1"}), \
             patch("openjob.executor.sender._message_delivery_state", return_value="missing"), \
             patch("openjob.executor.sender._submit_chat_message_background", return_value={"success": True}), \
             patch("openjob.executor.sender._verify_greeting_in_chat_list", return_value=True), \
             patch("openjob.executor.sender.close_tab") as close_tab, \
             patch("openjob.executor.sender.time.sleep"):
            result, target_id = _send_greeting_once(
                job,
    ...
    def test_send_greeting_preset_popup_confirmed_without_duplicate_send(self):
        """弹窗不可编辑时：确认默认招呼语后立即结束，不得再走聊天框补发。"""
        job = {
            "id": "preset-popup",
            "company": "Example",
            "title": "Engineer",
        ...
        with patch("openjob.executor.sender.new_tab", return_value="target-1"),              patch("openjob.executor.sender.evaluate", return_value='{"success": true}'),              patch("openjob.executor.sender._click_chat_button", return_value={"success": True}),              patch(
                 "openjob.executor.sender._detect_greet_popup",
                 return_value={"success": True, "popup": True, "kind": "preset_greeting"},
             ),              patch(
                 "openjob.executor.sender._confirm_preset_greeting",
                 return_value={"success": True, "action": "preset_confirmed"},
             ) as confirm_preset,              patch(
                 "openjob.executor.sender._fill_chat_input",
                 return_value={"success": True, "disabled": False},
             ) as fill_input,              patch(
                 "openjob.executor.sender._submit_chat_message_background",
                 return_value={"success": True, "action": "chat_submitted_background"},
             ) as background_submit,              patch("openjob.executor.sender.click_at", return_value=True),              patch("openjob.executor.sender.close_tab") as close_tab,              patch("openjob.executor.sender.time.sleep"):
                result, target_id = _send_greeting_once(
                    job,
                    "定制招呼语。",
                    {"browse_before_greet": False},
        ...
        self.assertIsNone(target_id)
        self.assertTrue(result["success"])
        self.assertIn("默认招呼语", result.get("history_detail", ""))
        fill_input.assert_not_called()
        background_submit.assert_not_called()
        close_tab.assert_called()

    def test_send_greeting_preset_popup_replaces_text_with_tailored_greeting(self):
        """弹窗可编辑时：替换为定制文案再确认，action=first_contact_submitted。"""
        job = {
            "id": "preset-popup-edit",
            "company": "Example",
            "title": "Engineer",
            "url": "https://www.zhipin.com/job_detail/preset-popup-edit.html",
        }
        ...
        with patch("openjob.executor.sender.new_tab", return_value="target-1"),              patch("openjob.executor.sender.evaluate", return_value='{"success": true}'),              patch("openjob.executor.sender._click_chat_button", return_value={"success": True}),              patch(
                 ...
                 "openjob.executor.sender._confirm_preset_greeting",
                 return_value={"success": True, "action": "first_contact_submitted", "preset_replaced": True},
             ),              patch(
                 "openjob.executor.sender._wait_for_chat_page",
                 return_value={"success": True, "target_id": "target-1"},
             ),              patch(
                 "openjob.executor.sender._message_delivery_state",
                 ...
                 "openjob.executor.sender._submit_chat_message_background",
                 return_value={"success": True, "action": "chat_submitted_background"},
             ) as background_submit,              patch("openjob.executor.sender.click_at", return_value=True),              patch("openjob.executor.sender.close_tab"),              patch("openjob.executor.sender.time.sleep"):
                result, target_id = _send_greeting_once(
                    job,
                    "定制招呼语。",
                    {"browse_before_greet": False},
        ...
        self.assertIsNone(target_id)
        self.assertTrue(result["success"])
        self.assertTrue(result.get("first_contact"))
        fill_input.assert_not_called()
        background_submit.assert_not_called()
    ...
    def test_send_greeting_redirects_first_contact_and_uses_background_chat_submit(self):
        ...
        with patch("openjob.executor.sender.new_tab", return_value="target-1"), \
             patch("openjob.executor.sender.evaluate", return_value='{"success": true}'), \
             patch(
                 "openjob.executor.sender._click_chat_button",
                 return_value={
                     "success": True,
             ...
             patch("openjob.executor.sender._submit_startchat_greeting") as foreground_submit, \
             patch(
                 "openjob.executor.sender._wait_for_chat_page",
                 return_value={"success": True, "target_id": "target-1"},
             ), \
             ...
             patch("openjob.executor.sender._fill_chat_input") as fill_input, \
             patch("openjob.executor.sender.close_tab") as close_tab, \
             patch("openjob.executor.sender.time.sleep"):
            result, target_id = _send_greeting_once(
                job,
    ...
    def test_first_contact_without_chat_redirect_fails_without_foreground_click(self):
        ...
        with patch("openjob.executor.sender.new_tab", return_value="target-1"), \
             ...
             patch("openjob.executor.sender._submit_startchat_greeting") as foreground_submit, \
             patch("openjob.executor.sender.click_at") as foreground_click, \
             patch("openjob.executor.sender.time.sleep"):
            result, target_id = _send_greeting_once(
                job,
                "您好，我对这个岗位很感兴趣。",
    ...
    def test_wait_for_chat_page_rejects_a_different_job_conversation(self):
        ...
        with patch("openjob.executor.sender.evaluate", return_value="/web/geek/chat"), \
             patch("openjob.executor.sender._chat_target_matches_job", return_value=False) as matches_job, \
             patch("openjob.executor.sender.get_page_targets", return_value=[]), \
             patch("openjob.executor.sender.time.sleep"):
    ...
    def test_wait_for_chat_page_adopts_preexisting_chat_when_active_job_matches(self):
        ...
        with patch("openjob.executor.sender.evaluate", return_value="/job_detail/expected-job"), \
             patch("openjob.executor.sender._chat_target_matches_job", return_value=True) as matches_job, \
             patch("openjob.executor.sender.get_page_targets", return_value=targets), \
             patch("openjob.executor.sender.time.sleep"):
    ...
    def test_send_greeting_adopts_matching_new_chat_tab_and_closes_old_tab(self):
        ...
        with patch("openjob.executor.sender.new_tab", return_value="job-target"), \
             patch("openjob.executor.sender.evaluate", return_value='{"success": true}'), \
             patch("openjob.executor.sender._click_chat_button", return_value={"success": True}), \
             patch("openjob.executor.sender._detect_greet_popup", return_value={"success": True, "popup": False}), \
             patch(
    ...
    def test_send_greetings_reopens_job_page_once_when_chat_input_missing(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("openjob.db.DB_PATH", db_path), \
                 patch("openjob.executor.sender.should_take_day_off", return_value=False), \
                 patch("openjob.executor.sender.SendWindowChecker.is_active", return_value=True), \
                 patch("openjob.executor.sender._send_greeting_once", side_effect=attempts) as send_once, \
                 patch("openjob.executor.sender.close_tab") as close_tab:
    ...
    def test_send_greetings_closes_failed_task_page_after_recording_error(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("openjob.db.DB_PATH", db_path), \
                 ...
                 patch("openjob.executor.sender.SendWindowChecker.is_active", return_value=True), \
                 patch(
                     "openjob.executor.sender._send_greeting_once",
    ...
    def test_send_greetings_force_bypasses_send_window_restriction(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            # Act
            with patch("openjob.db.DB_PATH", db_path), \
                 patch("openjob.throttle.datetime") as mock_datetime, \
                 patch("openjob.executor.sender._send_greeting_once", return_value=({"success": True}, None)):
