...
from openjob.web.server import _execute_collect
...
class ScraperBackgroundTests(unittest.TestCase):
    def test_stopped_collection_does_not_open_a_search_page(self):
        ...
        with patch("openjob.scraper.jobs.get_db", return_value=db), \
             patch("openjob.scraper.jobs.new_tab") as new_tab:
    ...
    def test_workbench_passes_its_stop_event_into_collection(self):
        ...
        with patch("openjob.scraper.jobs.scrape_jobs", side_effect=scrape_with_progress) as scrape, \
             patch("openjob.ai.scorer.score_jobs", side_effect=score_with_progress):
    ...
    def test_scraper_reports_seen_new_and_duplicate_counts(self):
        ...
        with patch("openjob.scraper.jobs.get_db", return_value=db), \
             patch("openjob.collection.platforms.boss.PlatformAccessGuard") as guard_cls, \
             patch("openjob.scraper.jobs.Progress", return_value=progress_context), \
             patch("openjob.scraper.jobs.PageThrottle") as throttle_cls, \
             patch("openjob.scraper.jobs.new_tab", return_value="worker-target"), \
             patch("openjob.scraper.jobs.navigate", return_value=True), \
             patch("openjob.scraper.jobs.evaluate", side_effect=[
                 json.dumps({"risk": None}), json.dumps(jobs),
                 json.dumps({"risk": None}), json.dumps(detail),
             ]), \
             patch("openjob.scraper.jobs.wait_for_load"), \
             patch("openjob.scraper.jobs.scroll"), \
             patch("openjob.scraper.jobs.close_tab"), \
             patch("openjob.scraper.jobs.job_exists", side_effect=[True, False]), \
             patch("openjob.scraper.jobs.matching_deal_breaker", return_value=False), \
             patch("openjob.scraper.jobs.insert_job"), \
             patch("openjob.scraper.jobs.time.sleep"):
    ...
    def test_search_and_detail_pages_reuse_one_background_worker_tab(self):
        ...
        with patch("openjob.scraper.jobs.get_db", return_value=db), \
             ...
             patch("openjob.scraper.jobs.PageThrottle") as throttle_cls, \
             patch(
                 "openjob.scraper.jobs.new_tab",
                 return_value="worker-target",
             ) as new_tab, \
             patch("openjob.scraper.jobs.navigate", return_value=True) as navigate, \
             patch(
                 "openjob.scraper.jobs.evaluate",
                 side_effect=[
                     json.dumps({"risk": None}), json.dumps(jobs),
                     json.dumps({"risk": None}), json.dumps(detail),
                 ],
             ...
             patch("openjob.scraper.jobs.scroll"), \
             patch("openjob.scraper.jobs.close_tab"), \
             patch("openjob.scraper.jobs.job_exists", return_value=False), \
             patch("openjob.scraper.jobs.matching_deal_breaker", return_value=False), \
             patch("openjob.scraper.jobs.insert_job"), \
             patch("openjob.scraper.jobs.time.sleep"):
            throttle_cls.return_value.wait.return_value = None
