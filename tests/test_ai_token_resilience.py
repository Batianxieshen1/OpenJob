...
from openjob.ai import credentials, greeter, scorer
...
class AiCredentialErrorTests(unittest.TestCase):
    def test_openai_compatible_quota_error_is_not_silently_swallowed(self):
        ...
        with patch("openjob.ai.credentials.httpx.post", return_value=response):
...
class ScorerTokenResilienceTests(unittest.TestCase):
    ...
    def test_borderline_structured_score_is_reviewed_and_averaged(self):
        ...
        with (
            patch("openjob.ai.scorer.get_db", return_value=db),
            patch("openjob.ai.scorer._load_resume", return_value="真实简历"),
            patch("openjob.ai.scorer.get_jobs_by_status", return_value=[job]),
            patch("openjob.ai.scorer.quick_score", return_value=(80, "通过")),
    ...
    def test_ai_calls_run_with_configured_concurrency_but_db_writes_stay_on_main_thread(self):
        ...
        with (
            patch("openjob.ai.scorer.get_db", return_value=db),
            patch("openjob.ai.scorer._load_resume", return_value="真实简历"),
            patch("openjob.ai.scorer.get_jobs_by_status", return_value=jobs),
            patch("openjob.ai.scorer.quick_score", return_value=(80, "通过")),
    ...
    def test_stop_returns_without_waiting_for_inflight_concurrent_ai_calls(self):
        ...
        with (
            patch("openjob.ai.scorer.get_db", return_value=db),
            patch("openjob.ai.scorer._load_resume", return_value="真实简历"),
            patch("openjob.ai.scorer.get_jobs_by_status", return_value=[_job("1"), _job("2")]),
            patch("openjob.ai.scorer.quick_score", return_value=(80, "通过")),
    ...
    def test_scoring_processes_all_unscored_pending_jobs(self):
        ...
        with (
            patch("openjob.ai.scorer.get_db", return_value=db),
            patch("openjob.ai.scorer._load_resume", return_value="真实简历"),
            patch("openjob.ai.scorer.get_jobs_by_status", return_value=[old_job, new_job]),
            patch("openjob.ai.scorer.quick_score", return_value=(80, "通过")),
            patch(
                "openjob.ai.scorer._call_claude",
    ...
    def test_context_limit_retries_once_with_compact_prompt(self):
        ...
        with (
            patch("openjob.ai.scorer.get_db", return_value=db),
            patch("openjob.ai.scorer._load_resume", return_value="简历内容" * 1000),
            patch("openjob.ai.scorer.get_jobs_by_status", return_value=[job]),
            patch("openjob.ai.scorer.quick_score", return_value=(80, "通过")),
            patch(
                "openjob.ai.scorer._call_claude",
...
class GreeterTokenResilienceTests(unittest.TestCase):
    ...
    def test_portfolio_is_only_added_when_the_job_explicitly_requests_it(self):
        ...
        with patch("openjob.ai.greeter._call_claude", return_value="生成结果") as call_ai:
    ...
    def test_invalid_review_format_keeps_the_generated_greeting(self):
        ...
        with (
            patch("openjob.ai.greeter.get_db", return_value=db),
            patch("openjob.ai.greeter.get_jobs_by_status", return_value=jobs),
            patch("openjob.ai.greeter._get_resume_summary", return_value="真实简历摘要"),
            patch(
                "openjob.ai.greeter._call_claude",
                side_effect=[
                    "这是一条可用的个性化招呼语。",
                    "评分很好，但没有按 JSON 返回。",
                ],
    ...
    def test_style_guard_rewrites_even_when_model_review_is_malformed(self):
        ...
        with (
            patch("openjob.ai.greeter.get_db", return_value=db),
            patch("openjob.ai.greeter.get_jobs_by_status", return_value=jobs),
            patch("openjob.ai.greeter._get_resume_summary", return_value="匿名简历摘要"),
            patch(
                "openjob.ai.greeter._call_claude",
                side_effect=[
