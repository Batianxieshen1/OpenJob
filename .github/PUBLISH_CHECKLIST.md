...
**问题**：项目名包含 "某直聘" 商标
**解决**：
- [x] 对外名称改为 `OpenJob`（中性名称）
- [x] README 免责声明放在最顶部
- [x] 代码中移除 "openjob" → 改为 "openjob"（包名、模块名）
...
### 代码层面
- [x] 全局替换 `openjob` → `openjob`（包名、import、CLI 名）
...
### 前端层面
- [ ] 构建 production 版本：`cd src/openjob/web/frontend && npm run build`
...
| 原名 | 新名 | 位置 |
|------|------|------|
| openjob | openjob | Python 包名 |
| OpenJob | OpenJob | 项目显示名 |
| openjob.db | openjob.db | 数据库文件名 |
| `openjob` CLI | `openjob` CLI | 命令行入口 |
| src/openjob/ | src/openjob/ | 源码目录 |
