"""Configuration loader for OpenJob."""
...
def load_config(config_path: Path | None = None) -> dict[str, Any]:
    ...
    if config_path.exists():
        ...
        if isinstance(user_cfg, dict):
            _infer_legacy_ai_service(user_cfg)
...
def _infer_legacy_ai_service(user_cfg: dict[str, Any]) -> None:
    """Preserve pre-service-field configs before defaults can shadow them."""
    ai_cfg = user_cfg.get("ai")
    if not isinstance(ai_cfg, dict) or ai_cfg.get("service"):
        return
    provider = ai_cfg.get("provider")
    if provider == "openai_compatible":
        ai_cfg["service"] = "custom"
    elif provider == "anthropic":
        ai_cfg["service"] = "anthropic"
