...
from openjob.throttle import RequestThrottle, SendWindowChecker, ProgressiveBackoff, should_take_day_off
...
def _confirm_preset_greeting(target_id: str, greeting: str = "") -> dict:
    """确认平台预设招呼语弹窗。

    优先在弹窗输入框中替换为定制文案（发出的是定制招呼语，action=first_contact_submitted）；
    弹窗不可编辑时直接确认（BOSS 将发送账号默认招呼语，action=preset_confirmed，
    由调用方终止后续发送，避免同会话出现两条招呼语）。
    """
    if greeting:
        greeting_escaped = json.dumps(greeting, ensure_ascii=False)
        input_state = _parse_js_result(evaluate(target_id, f"""
        (() => {{
            ...
            const popup = Array.from(document.querySelectorAll('.greet-boss-pop, .greet-pop, .dialog-wrap'))
                .filter(visible)
                .find((element) =>
                    element.matches('.greet-boss-pop, .greet-pop')
                    || /预设招呼语|默认招呼语|自动招呼语|打招呼语/.test((element.innerText || '').trim())
                );
            ...
            if (!input) return JSON.stringify({{success: false, error: 'preset_input_missing'}});
            input.scrollIntoView({{block: 'center', inline: 'center'}});
            const rect = input.getBoundingClientRect();
            return JSON.stringify({{success: true, x: rect.x + rect.width / 2, y: rect.y + rect.height / 2}});
        }})()
        """))
        if input_state.get("success"):
            focused = click_at(target_id, f"{input_state['x']},{input_state['y']}")
            cleared = focused and press_key(target_id, "SelectAll") and press_key(target_id, "Backspace")
            typed = cleared and type_text(target_id, greeting, human=True)
            if typed:
                verified = _parse_js_result(evaluate(target_id, f"""
                (() => {{
                    ...
                    const value = input
                        ? (input.value !== undefined ? input.value : (input.innerText || ''))
                        : '';
                    if (normalize(value) !== normalize({greeting_escaped})) {{
                        return JSON.stringify({{success: false, error: 'preset_input_not_replaced'}});
                    }}
                ...
                """))
                if verified.get("success") and click_at(target_id, f"{verified['x']},{verified['y']}"):
                    return {"success": True, "action": "first_contact_submitted", "preset_replaced": True}
...
def _submit_chat_message_background(target_id: str, greeting: str) -> dict:
    """Use OpenJob's original Vue submit path without foregrounding Chrome."""
    greeting_escaped = json.dumps(greeting, ensure_ascii=False)
    result = _parse_js_result(evaluate(target_id, f"""
    (() => {{
...
def _has_outgoing_messages(target_id: str) -> bool:
    """Return True when the open conversation already contains a candidate-sent message.

    复用监听器的会话提取与发送者分类（senderOf），保证两处对“我方消息”的判定一致。
    提取失败时按无我方消息处理（fail-open），不阻塞正常发送。
    """
    import json as _json

    from openjob.executor.monitor import JS_EXTRACT_CONVERSATION
    ...
    if isinstance(data, list):
        ...
        items = []
    for item in items:
        if isinstance(item, dict) and str(item.get("sender") or "") == "me":
            return True
    return False
...
def _send_greeting_once(job: dict, greeting: str, throttle_config: dict) -> tuple[dict, str | None]:
    ...
    if contact_action == "preset_confirmed":
        # 弹窗不可编辑：BOSS 已发送账号默认招呼语，到此为止，不再补发定制文案，
        # 避免同一会话出现两条招呼语。
        close_tab(target_id)
        console.print("[yellow]    ! 平台招呼语弹窗不可编辑，已确认发送账号默认招呼语（未重复发送定制文案）[/yellow]")
        return {
            "success": True,
            "verified": True,
            "first_contact": True,
            "history_detail": "平台招呼语弹窗不可编辑，已确认发送账号默认招呼语（未替换为定制文案）",
        }, None
    ...
    # 会话级去重：只要会话里已存在我方发送的任何消息（例如此前手动打过招呼），
    # 就不再发送第二条招呼语，避免 HR 收到重复内容。
    # 走到这里时会话页已完全加载（wait_for_chat_page 成功后），单次检查足够。
    if _has_outgoing_messages(target_id):
        close_tab(target_id)
        console.print("[yellow]    ! 会话中已存在我方消息，跳过重复打招呼[/yellow]")
        return {
            "success": True,
            "already_present": True,
            "verified": True,
            "history_detail": "会话中已存在我方发送的消息（可能此前已手动打过招呼），系统未重复发送，按已完成记录",
        }, None
...
def send_greetings(config: dict, force: bool = False) -> int:
    ...
    if not jobs:
        console.print("[yellow]没有已生成招呼语的待发送岗位，请先运行 openjob greet[/yellow]")
