...
from openjob.ai.credentials import call_anthropic_text
from openjob.browser import new_tab, close_tab, evaluate, click, wait_for_load, get_page_info
from openjob.cancellation import (
    OperationCancelled,
    get_stop_event,
    run_cancellable,
    stop_requested,
...
def _handle_conversation(job: dict, config: dict, conversation: dict | None = None) -> str:
    ...
    if _detect_resume_request(messages):
        ...
        from openjob.ai.resume import generate_tailored_resume, get_last_resume_failure_reason
        ...
        try:
            ...
            notify_desktop(
                f"HR 索要简历：{job.get('company') or ''}",
                f"定制简历已生成，请打开面板「监测执行」手动发送给 {job.get('company') or ''}",
                config,
            )
        except Exception:
            pass
