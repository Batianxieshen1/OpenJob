"""OpenJob Web Server - Bottle HTTP service + API routes.
...
from openjob.ai.credentials import get_ai_api_key
...
from openjob.job_export import InvalidJobSelectionError, export_jobs, export_row_count
...
def _sanitize_config_for_write(data):
	...
	if service not in AI_SERVICE_PRESETS:
		provider = ai_cfg.get("provider") or existing_ai.get("provider") or "openai_compatible"
		service = "custom" if provider == "openai_compatible" else "deepseek"
...
def _preflight_messages(mode: str, config: dict, options: dict | None = None) -> list[str]:
	...
	if mode == "collect":
		...
		if collection_options.get("auto_score"):
			if not _has_any_resume_source():
				messages.append("自动评分前请先在配置页上传简历，或上传至少一份多方向底稿。")
	...
	if mode in {"full", "rescore"} and not _has_any_resume_source():
		messages.append("请先在配置页上传简历，或上传至少一份多方向底稿。")
...
def _execute_collect(task: WorkbenchTask, config: dict) -> None:
	...
	if "_collection_options" not in config:
		...
		from openjob.scraper.jobs import scrape_jobs
...
def _execute_monitor(task: WorkbenchTask, config: dict, *, initial_cooldown: bool = False) -> None:
	from openjob.executor.monitor import (
...
def _execute_deliver_batch(task: WorkbenchTask, config: dict) -> None:
	...
	if sent_count:
		try:
			...
			notify_desktop(
				f"投递完成：成功 {sent_count} 条",
				f"失败 {failed_count}，待下次发送 {deferred_count}；今日剩余额度 {int(report.get('remaining_quota', 0) or 0)} 封",
				config,
			)
		except Exception:
			pass
...
def api_market_stats():
	scope = request.params.get("scope", "all").strip()
	if scope not in {"all", "pass", "filtered"}:
		scope = "all"
	where = {
		"all": "deleted_at IS NULL",
		"pass": "deleted_at IS NULL AND status IN ('ready', 'approved', 'sent', 'replied', 'needs_resume', 'resume_sent', 'follow_up_sent')",
		"filtered": "deleted_at IS NULL AND status = 'filtered'",
	}[scope]
	db = _get_web_db()
	try:
		rows = db.execute(
	...
	result = compute_market_stats([dict(r) for r in rows])
	result["scope"] = scope
...
def api_usage_summary():
	...
	return _json_response(summarize_usage(days))
...
def api_scoring_runs():
	return _json_response(list_scoring_runs(DATA_DIR / "openjob.db"))
...
def api_workbench_task_start():
	try:
		...
		if mode == "collect":
			...
			try:
				from openjob.db import backup_database
...
def api_collection_runs():
	try:
		...
		return _json_response(list_collection_runs(DATA_DIR / "openjob.db", limit=limit))
	except (TypeError, ValueError) as exc:
		return _json_response({"error": str(exc)}, 400)
...
def api_collection_run_detail(run_id):
	run = get_collection_run(DATA_DIR / "openjob.db", run_id)
	if not run:
...
def api_workbench_reject():
	try:
		...
		block_companies = sorted({
			str(name).strip()
			for name in (body.get("block_companies") or [])
			if str(name).strip()
		})
		...
		if block_companies:
			...
			existing = {str(name) for name in profile.get("blocked_companies") or []}
			profile["blocked_companies"] = sorted(existing | set(block_companies))
			blocked_new = len(profile["blocked_companies"]) - len(existing)
...
@app.route("/api/jobs/<job_id>/approve", method="POST")
def api_job_approve(job_id):
	"""人工放行：用户判断优先于 AI 评分，把被过滤岗位放回确认队列。"""
	db = _get_web_db()
	try:
		row = db.execute(
...
def api_resume_version_download(resume_id):
	...
	format_requested = request.params.get("format", "pdf").strip().lower()
	db = _get_web_db()
	try:
		row = db.execute(
			"SELECT pdf_path, docx_path, content_md FROM resumes WHERE id = ?", (resume_id,)
		).fetchone()
	finally:
		db.close()
	if not row:
		return _json_response({"error": "简历版本不存在"}, 404)
	...
	elif format_requested == "md":
		if row["content_md"]:
			path = save_markdown(row["content_md"], RESUME_DIR / f"resume_{resume_id}.md")
	else:
		path = Path(row["pdf_path"]) if row["pdf_path"] else None

	if not path or not path.exists():
		return _json_response({"error": f"{format_requested.upper()} 文件不存在（可能未导出）"}, 404)
	return static_file(path.name, root=str(path.parent), download=path.name)


@app.route("/api/jobs/<job_id>/verify-welfare")
def api_job_verify_welfare(job_id):
	"""福利核验：逐个关键词查 JD 原文并摘录证据（V1 迁入）。

	列表标签可能不完整/不真实，福利必须以 JD 原文为准。
	"""
	welfare_raw = request.params.get("welfare", "")
	keywords = [w.strip() for w in welfare_raw.replace("，", ",").split(",") if w.strip()]
	if not keywords:
		return _json_response({"error": "请提供要核验的福利关键词，逗号分隔"}, 400)

	db = _get_web_db()
	try:
		row = db.execute("SELECT jd FROM jobs WHERE id = ? AND deleted_at IS NULL", (job_id,)).fetchone()
	finally:
		db.close()
	if not row:
		return _json_response({"error": "岗位不存在"}, 404)
	jd_text = str(row["jd"] or "")
	if not jd_text.strip():
		return _json_response({"error": "该岗位没有 JD 原文，无法核验"}, 404)

	results = []
	for kw in keywords:
		idx = jd_text.find(kw)
		if idx >= 0:
			snippet = jd_text[max(0, idx - 20): idx + len(kw) + 30].replace("\n", " ")
			results.append({"keyword": kw, "found": True, "evidence": snippet})
		else:
			results.append({"keyword": kw, "found": False, "evidence": ""})
...
def api_resume_bases_list():
	db = _get_web_db()
	try:
		return _json_response({"bases": list_base_resumes(db)})
	finally:
		db.close()


@app.route("/api/resume/bases", method="POST")
def api_resume_bases_upload():
	"""上传一份底稿：multipart(file, name, direction, is_default)。"""
	try:
		upload = request.files.get("file")
	except Exception:
		upload = None
	if upload is None or not upload.filename:
		return _json_response({"error": "请选择简历文件（md/docx/带文字层 pdf）"}, 400)

	name = (request.forms.get("name") or "").strip()
	direction = (request.forms.get("direction") or "").strip()
	is_default = str(request.forms.get("is_default") or "").lower() in {"1", "true", "yes"}

	_original_bytes = upload.file.read()
	try:
		_stored_name, content = prepare_resume_content(upload.filename, _original_bytes)
	...
	if Path(upload.filename).suffix.lower() == ".docx":
		...
		try:
			existing = db.execute(
				"SELECT id FROM base_resumes WHERE name = ? OR (direction != '' AND direction = ?) LIMIT 1",
				(display_name, direction),
			).fetchone()
		finally:
			db.close()

	upload_is_docx = Path(upload.filename).suffix.lower() == ".docx"

	db = _get_web_db()
	try:
		if upload_is_docx and existing:
			# 同名/同方向重传 → 更新内容与模板，不新增
			base_id = str(existing["id"])
			from openjob.db import update_base_resume as _update_base

			_update_base(db, base_id, content_md=content.decode("utf-8", errors="replace") if isinstance(content, bytes) else content)
			template_path = RESUME_DIR / "templates" / f"{base_id}.docx"
			template_path.parent.mkdir(parents=True, exist_ok=True)
			template_path.write_bytes(_original_bytes)
			_update_base(db, base_id, template_path=str(template_path))
			return _json_response({"success": True, "id": base_id, "name": display_name, "updated": True})

		base_id = str(uuid4())
		insert_base_resume(db, base_id=base_id, name=display_name, direction=direction,
			content_md=content.decode("utf-8", errors="replace") if isinstance(content, bytes) else content,
		...
		if upload_is_docx:
			...
			_update_base(db, base_id, template_path=str(template_path))
		return _json_response({"success": True, "id": base_id, "name": display_name})
	finally:
		db.close()


@app.route("/api/resume/bases/<base_id>", method="PATCH")
def api_resume_bases_update(base_id):
	try:
		data = request.json
	except Exception:
	...
	db = _get_web_db()
	try:
		updated = update_base_resume(
			db,
			base_id,
			name=(str(data["name"]).strip() or None) if "name" in data else None,
			direction=(str(data["direction"]).strip()) if "direction" in data else None,
			is_default=bool(data["is_default"]) if "is_default" in data else None,
		)
		if not updated:
			return _json_response({"error": "底稿不存在"}, 404)
		return _json_response({"success": True})
	finally:
		db.close()


@app.route("/api/resume/bases/<base_id>", method="DELETE")
def api_resume_bases_delete(base_id):
	db = _get_web_db()
	try:
		if not delete_base_resume(db, base_id):
			return _json_response({"error": "底稿不存在"}, 404)
		return _json_response({"success": True})
	finally:
		db.close()

@app.route("/api/resume/jobs-with-versions")
def api_resume_jobs_with_versions():
	"""有定制简历版本的岗位列表（供简历工作台下拉置顶）。"""
	db = _get_web_db()
	try:
		rows = db.execute(
			"SELECT DISTINCT j.id, j.title, j.company, COUNT(r.id) AS version_count "
			"FROM resumes r JOIN jobs j ON j.id = r.job_id "
			"WHERE j.deleted_at IS NULL "
			"GROUP BY j.id, j.title, j.company ORDER BY version_count DESC, j.created_at DESC"
		).fetchall()
	finally:
		db.close()
	return _json_response({"jobs": [dict(r) for r in rows]})


@app.route("/api/resume/generate", method="POST")
def api_resume_generate():
	"""触发定制简历生成（幂等：已有 review 版返回既有；重新生成需 confirm）。"""
	try:
		data = request.json
	except Exception:
		return _json_response({"error": "请求体必须是 JSON"}, 400)
	if not isinstance(data, dict) or not str(data.get("job_id") or "").strip():
		return _json_response({"error": "缺少 job_id"}, 400)

	job_id = str(data["job_id"]).strip()
	confirm = bool(data.get("confirm_regenerate"))
	config = load_config(CONFIG_PATH)

	from openjob.ai.resume_engine import generate_resume

	try:
		result = generate_resume(job_id, config, confirm_regenerate=confirm)
	except OperationCancelled:
		return _json_response({"error": "任务已取消"}, 409)
	except Exception as exc:
		reason = getattr(exc, "user_message", None) or getattr(exc, "reason", None) or str(exc)
		return _json_response({"error": reason}, 400)
	if not result.ok:
		return _json_response({"error": result.reason or "生成失败"}, 400)
	return _json_response({
		"success": True,
...
@app.route("/api/resume/<job_id>/versions")
def api_resume_versions(job_id):
	db = _get_web_db()
	try:
		...
		if not row:
			return _json_response({"error": "岗位不存在"}, 404)
		rows = db.execute(
			"SELECT id, version, status, pdf_path, risk_flags_json, error, created_at, updated_at "
			"FROM resumes WHERE job_id = ? ORDER BY version DESC",
			(job_id,),
		).fetchall()
		return _json_response({"versions": [dict(r) for r in rows]})
	finally:
		db.close()


@app.route("/api/resume/version/<resume_id>")
def api_resume_version_detail(resume_id):
	db = _get_web_db()
	try:
...
@app.route("/api/resume/version/<resume_id>", method="PATCH")
def api_resume_version_patch(resume_id):
	"""逐块采纳/撤销/手改：提交完整 diff，服务端基于 base_md 重新合成。"""
	try:
		data = request.json
	except Exception:
		return _json_response({"error": "请求体必须是 JSON"}, 400)
	if not isinstance(data, dict) or not isinstance(data.get("diff"), list):
		return _json_response({"error": "缺少 diff 数组"}, 400)

	db = _get_web_db()
	try:
		record = db.execute("SELECT * FROM resumes WHERE id = ?", (resume_id,)).fetchone()
		if not record:
			return _json_response({"error": "简历版本不存在"}, 404)
		if record["status"] not in ("review", "exported"):
		...
		risk_flags = sorted({item["risk"] for item in diff if item["adopted"] and item["risk"]} | set(warnings))
		update_resume_version(
			db,
			resume_id,
		...
		set_job_resume_pointer(db, record["job_id"], resume_id=resume_id, resume_status="review")
		return _json_response({"success": True, "content_md": assembled, "risk_flags": risk_flags})
	finally:
		db.close()


@app.route("/api/resume/version/<resume_id>/export", method="POST")
def api_resume_version_export(resume_id):
	try:
		body = request.json
	except Exception:
		body = {}
	fmt = str((body or {}).get("format") or "both").strip().lower()
	if fmt not in {"pdf", "docx", "md", "both"}:
		return _json_response({"error": "format 只支持 pdf / docx / md / both"}, 400)

	db = _get_web_db()
	try:
		record = db.execute("SELECT status FROM resumes WHERE id = ?", (resume_id,)).fetchone()
		if not record:
			return _json_response({"error": "简历版本不存在"}, 404)
		if record["status"] not in ("review", "exported"):
			return _json_response({"error": f"当前状态 {record['status']} 不可导出"}, 409)
	finally:
		db.close()

	from openjob.ai.resume_engine import export_resume_pdf

	try:
		path = export_resume_pdf(resume_id, load_config(CONFIG_PATH), fmt=fmt)
	except Exception as exc:
		return _json_response({"error": f"导出失败：{exc}"}, 500)
	if path is None:
		return _json_response({"error": "导出失败：缺少简历内容"}, 400)
	return _json_response({"success": True, "path": str(path)})


@app.route("/api/resume/version/<resume_id>/mark-sent", method="POST")
def api_resume_version_mark_sent(resume_id):
	try:
		data = request.json
	except Exception:
		data = {}
	...
	db = _get_web_db()
	try:
		from openjob.ai.resume_engine import mark_resume_sent

		if not mark_resume_sent(resume_id, job_id, "Web Dashboard 标记定制简历已发送"):
			return _json_response({"error": "简历版本与岗位不匹配"}, 404)
		return _json_response({"success": True})
	finally:
		db.close()
...
class _GzipMiddleware:
	"""WSGI 中间件：对 JS/CSS/JSON/SVG 响应做 gzip，静态资源传输体积约降 3 倍。"""

	_COMPRESSIBLE = ("javascript", "css", "json", "svg")
	_MIN_SIZE = 1024
	...
	def __call__(self, environ, start_response):
		accepts = environ.get("HTTP_ACCEPT_ENCODING", "")
		...
		body = self.wrapped(environ, sr)
		try:
			body = b"".join(body)
		except Exception:
			start_response(status_headers["status"], status_headers["headers"])
			return [body] if isinstance(body, bytes) else [b""]

		headers = dict(status_headers["headers"])
		content_type = headers.get("Content-Type", "")
		if not any(t in content_type for t in self._COMPRESSIBLE) or len(body) < self._MIN_SIZE:
			start_response(status_headers["status"], status_headers["headers"])
			return [body]
		...
		out_headers = [(k, v) for k, v in status_headers["headers"] if k.lower() not in ("content-length",)]
		out_headers.append(("Content-Encoding", "gzip"))
		out_headers.append(("Content-Length", str(len(compressed))))
		start_response(status_headers["status"], out_headers)
		return [compressed]
