      ...
      <div className="w-full max-w-xl rounded-3xl border border-card-border bg-card p-6 shadow-2xl">
        ...
        <div className="mt-5 space-y-4">
          <label className="block text-xs font-bold text-muted">
            评分范围
            <select value={scope} onChange={event => setScope(event.target.value as ScoreScope)} className="mt-1 w-full rounded-xl border border-card-border bg-surface-hover px-3 py-2 text-sm text-foreground outline-none focus:border-primary">
              {Object.entries(scopeLabels).map(([value, label]) => <option value={value} key={value}>{label}</option>)}
            </select>
          </label>
          <label className="block text-xs font-bold text-muted">
            数量（输入正整数，或输入“全部”）
            <input value={limitText} onChange={event => setLimitText(event.target.value)} className="mt-1 w-full rounded-xl border border-card-border bg-surface-hover px-3 py-2 text-sm text-foreground outline-none focus:border-primary" />
          </label>
          {scope === 'selected' && <div className="rounded-xl bg-accent-soft px-3 py-2 text-sm text-primary">岗位池已选择 {selectedJobIds.length} 条</div>}
          {preview && (
          ...
          <div className="rounded-2xl border border-warning/30 bg-warning/10 p-3 text-xs leading-5 text-warning">
          ...
          {message && <div className="rounded-xl bg-accent-soft px-3 py-2 text-sm text-primary">{message}</div>}
