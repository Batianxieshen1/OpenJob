  ...
  return (
    <div className="rounded-3xl border border-card-border bg-card p-5">
      <div className="mb-4 flex flex-wrap items-start justify-between gap-4">
        <div>
          <div className="flex items-center gap-2"><Trash2 className="h-5 w-5 text-primary" /><h2 className="text-2xl font-semibold">回收站</h2></div>
          <p className="mt-1 text-sm text-muted">保留岗位状态、评分、招呼语和历史；恢复后也不会自动启动评分或投递。</p>
        </div>
        <span className="rounded-full bg-accent-soft px-3 py-2 text-xs font-semibold text-primary">{jobs.length} 条</span>
      </div>
      {loading ? <div className="rounded-2xl border border-card-border bg-surface-hover p-6 text-center text-sm text-muted">正在读取回收站...</div> : (
        <>
