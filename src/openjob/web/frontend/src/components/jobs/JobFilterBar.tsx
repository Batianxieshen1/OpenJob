  ...
  return (
    <div className="mb-4 rounded-2xl border border-card-border bg-surface-hover p-3">
      ...
      <div className={expanded ? 'grid min-w-0 grid-cols-1 gap-2 md:grid-cols-2 2xl:grid-cols-4' : 'hidden min-w-0 grid-cols-1 gap-2 md:grid md:grid-cols-2 2xl:grid-cols-4'}>
        ...
        <Select className="min-w-0" value={filters.recruitmentType} onChange={event => update('recruitmentType', event.target.value)} aria-label="岗位性质">
          <option value="">岗位性质：全部</option>
          <option value="campus">只有实习</option>
          <option value="experienced">只有正式岗</option>
        </Select>
