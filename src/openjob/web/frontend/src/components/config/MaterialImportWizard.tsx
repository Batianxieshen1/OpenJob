...
type RowStatus = 'valid' | 'invalid' | 'example' | 'blank'

interface SheetMapping { source_column: string; target_field: string; confidence: number; reason: string; sample_values: string[] }
interface SheetInfo { name: string; hidden: boolean; header_row: number | null; columns: string[]; mapping: SheetMapping[]; row_count: number; valid_row_count?: number; suggested_include?: boolean; inferred_type?: MaterialType; warnings?: string[] }
...
interface SheetPreview { columns: string[]; mapping: SheetMapping[]; total_rows: number; valid_rows: number; invalid_rows: number; blank_rows: number; excluded_rows: number; rows: PreviewRow[]; page: number; total_pages: number }

const STANDARD_FIELDS = [
  ['__ignore__', '忽略此列'], ['id', 'ID'], ['type', '类型'], ['title', '标题（必填）'], ['organization', '组织/机构'], ['city', '城市/地点'], ['role', '角色/职位'], ['start_date', '开始时间'], ['end_date', '结束时间'], ['description', '描述（必填）'], ['achievements', '成果'], ['resume_bullets', '简历速写（独立保留）'], ['skills', '技能'], ['keywords', '关键词'], ['target_directions', '适用方向'], ['source', '来源（不进 AI）'], ['resume_allowed', '可用于简历'], ['priority', '优先级'], ['notes', '备注（不进 AI）'], ['certificate_number', '证书编号（不进 AI）'], ['award_level', '奖项级别'], ['award_ratio', '获奖比例'],
] as const
...
export function MaterialImportWizard({ onChanged }: { onChanged: () => void }) {
  ...
  const [result, setResult] = useState<{ count: number; excluded: number; ignored: number; warnings: string[] } | null>(null)
  ...
  const handleFile = async (file: File) => {
    ...
    try {
      ...
      setSheets((data.sheets || []).map((sheet: SheetInfo): SheetConfig => ({ ...sheet, include: sheet.suggested_include ?? (!sheet.hidden && sheet.header_row !== null), type_override: null, excluded_rows: [], page: 1 })))
  ...
  const excludeCurrentPageErrors = (sheet: SheetConfig) => {
    ...
    updateSheet(sheet.name, current => ({ ...current, excluded_rows: [...new Set([...current.excluded_rows, ...preview.rows.filter(row => row.status === 'invalid').map(row => row.excel_row)])].sort((a, b) => a - b) }))
  }
  const excludeAllErrors = async (sheet: SheetConfig) => {
    ...
    try {
      ...
      for (let page = 1; page <= preview.total_pages; page += 1) {
        ...
        errorRows.push(...(data.rows as PreviewRow[]).filter(row => row.status === 'invalid').map(row => row.excel_row))
      }
  ...
  const summary = useMemo(() => sheets.filter(sheet => sheet.include).reduce((total, sheet) => {
    ...
    return { valid: total.valid + preview.valid_rows, invalid: total.invalid + preview.invalid_rows, ignored: total.ignored + preview.blank_rows, excluded: total.excluded + preview.excluded_rows }
  }, { valid: 0, invalid: 0, ignored: 0, excluded: 0 }), [previews, sheets])
  ...
  return <div className="space-y-3"><input ref={fileRef} type="file" accept=".xlsx" className="hidden" onChange={event => { const file = event.target.files?.[0]; if (file) void handleFile(file) }} /><Button size="sm" disabled={phase === 'analyzing' || submitting} onClick={() => fileRef.current?.click()}>{phase === 'analyzing' ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />扫描中…</> : <><Upload className="mr-2 h-4 w-4" />导入自己的 Excel（智能识别）</>}</Button><p className="text-[11px] leading-4 text-muted-3">系统会在本地暂存文件中分析表头和字段；确认前可修正 Sheet、表头、映射、类型和错误行。不会编造缺失经历。</p>{error ? <div className="flex items-start gap-2 rounded-control border border-danger/30 bg-danger/10 px-3 py-2 text-xs text-danger"><AlertTriangle className="mt-0.5 h-3.5 w-3.5 shrink-0" />{error}<button type="button" aria-label="关闭错误提示" className="ml-auto" onClick={() => setError('')}><X className="h-3.5 w-3.5" /></button></div> : null}
    {(phase === 'mapping' || phase === 'importing') ? <div className="space-y-3">{warnings.map(warning => <div key={warning} className="rounded-control bg-warning/10 px-3 py-1.5 text-[11px] text-warning">{warning}</div>)}{sheets.map(sheet => {
      const preview = previews[sheet.name]; const rows = onlyErrors[sheet.name] ? preview?.rows.filter(row => row.status === 'invalid') : preview?.rows; const isLoading = loadingPreview[sheet.name]
